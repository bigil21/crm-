...
const checks = [
  ...
  ["estimate autosave keeps the editor active while explicit saves prevent duplicate clicks", !app.includes('setEstimateSaveState(estimateId, "Saving every estimate value') && app.includes("estimateExplicitSaves.has(estimate.id)") && app.includes('els.saveEstimateButton.disabled = true')],
  ...
  ["routine save notifications stay quiet", app.includes("isRoutineSaveNotice") && app.includes('tone === "success" && !options.showSuccess ? "" : message') && app.includes('{ showSuccess: true }') && workflowChecklists.includes('checklistSaveStates.set(saveKey, { message: "", tone: "" })')],
