(() => {
  ...
  function getState() {
    try { return window.state || state || null; } catch { return null; }
  }

  function callBool(name, fallback = false) {
    try { return typeof window[name] === "function" ? window[name]() === true : fallback; } catch { return fallback; }
  }

  function roleValue() {
    try { if (typeof window.currentRole === "function") return window.currentRole(); } catch {}
    try { if (typeof currentRole === "function") return currentRole(); } catch {}
    return getState()?.currentUser?.role || "viewer";
  }

  function canViewValue(view) {
    try { if (typeof window.canView === "function") return window.canView(view) === true; } catch {}
    try { if (typeof canView === "function") return canView(view) === true; } catch {}
    return false;
  }

  function canActionValue(action) {
    try { if (typeof window.canAction === "function") return window.canAction(action) === true; } catch {}
    try { if (typeof canAction === "function") return canAction(action) === true; } catch {}
    return false;
  }

  function setViewValue(view) {
    try { if (typeof window.setView === "function") { window.setView(view); return true; } } catch {}
    try { if (typeof setView === "function") { setView(view); return true; } } catch {}
    const current = getState();
    if (!current) return false;
    current.view = view;
    try { if (typeof window.render === "function") window.render(); else if (typeof render === "function") render(); } catch {}
    return true;
  }

  function openLeadDetailValue(contactId, tab) {
    try { if (typeof window.openLeadDetail === "function") { window.openLeadDetail(contactId, tab); return true; } } catch {}
    try { if (typeof openLeadDetail === "function") { openLeadDetail(contactId, tab); return true; } } catch {}
    return false;
  }
  ...
  function renderPanel() {
    ...
    panel.innerHTML = `
      ...
      <textarea id="directCheckJson" readonly>${escapeText(jsonPayload())}</textarea>
    `;
  ...
  async function copyResults() {
    ...
    if (box) { box.value = payload; box.focus(); box.select(); }
    try {
  ...
  async function waitForApp(timeoutMs = 18000) {
    ...
    while (Date.now() - started < timeoutMs) {
      if (document.querySelector("#globalSearch") && document.querySelector("#dashboardView") && getState()) return true;
      await sleep(250);
    }
    return false;
  }
  ...
  function readStorage(userId) {
    ...
    try { if (typeof activeStorageKey === "function") activeKey = activeStorageKey(); } catch { if (userId) activeKey = `roofline-crm-v1:${userId}`; }
  ...
  async function runCheck() {
    ...
    const email = String(auth?.user?.email || getState()?.currentUser?.email || "").toLowerCase();
    ...
    expectBoolean("Permission: Team Data", callBool("canManageTeamData"), shouldTeam, `Team-data permission matches ${role}.`, `Team-data permission is incorrect for ${role}.`);
    expectBoolean("Permission: Job Financials", callBool("canManageJobFinancials"), shouldFinancials, `Financial permission matches ${role}/${email}.`, `Financial permission is incorrect for ${role}/${email}.`);
